...
```
การฝากเช็กชื่อแบบเดิมส่วนใหญ่เกิดจากระบบ "ไร้การยืนยันตำแหน่งที่แท้จริง" (เช่น การส่งลิงก์หรือ QR Code ไปให้เพื่อนที่อยู่นอกห้องกดให้) ระบบนี้จึงแก้ปัญหาด้วยกลไก Double Validation

- ต้องอยู่ใกล้จริง (Physical Proximity via RSSI) เครื่อง ESP32 จะทำหน้าที่เป็น SoftAP และวัดค่าความแรงสัญญาณ Wi-Fi (RSSI) จากโทรศัพท์มือถือที่มาเชื่อมต่อ หากเพื่อนอยู่นอกห้องหรืออยู่ไกลเกินไป ค่า RSSI จะต่ำกว่าเกณฑ์ และระบบจะปฏิเสธการลงชื่อทันที

- ต้องกดยืนยันผ่าน HTTP Web Server เมื่อโทรศัพท์เข้ามาอยู่ในระยะ RSSI ที่ผ่านเกณฑ์แล้ว ผู้เรียนจึงจะสามารถเปิดหน้าเว็บที่ ESP32 ให้บริการขึ้นมา และกดปุ่ม Confirm Check-in ได้

- ผลลัพธ์ เพื่อนที่อยู่นอกห้อง ไม่สามารถลงชื่อแทนได้ เพราะแม้จะฝากเพื่อนในห้องเปิดหน้าเว็บแทน แต่ระบบจะตรวจจับ MAC Address และวัด RSSI ของเครื่องคนที่เปิดจริงเท่านั้น
```
2. เหตุใดระดับเกณฑ์ RSSI ที่ `-55 dBm` จึงเหมาะสมสำหรับการระบุตำแหน่งอุปกรณ์ให้อยู่ภายในรัศมีโต๊ะปฏิบัติการ?
```
- ระบุระยะใกล้ระดับ 1–2 เมตร โดยทั่วไป สัญญาณ Wi-Fi ที่ระยะชิดตัว (0–1 เมตร) จะมีความแรงอยู่ที่ประมาณ -30 ถึง -50 dBm การตั้งเกณฑ์ไว้ที่ -55 dBm จึงเป็นขอบเขต (Threshold) ที่การันตีว่าตัวเครื่องอยู่ติดกับโต๊ะปฏิบัติการที่มี ESP32 ตั้งอยู่จริงๆ

- กั้นสัญญาณข้ามโต๊ะ/ข้ามห้อง สัญญาณที่อ่อนกว่า -55 dBm (เช่น -60 ถึง -80 dBm) มักเกิดจากเครื่องที่อยู่ห่างออกไปมากกว่า 2–3 เมตร หรือมีสิ่งกีดขวาง/ตัวคนบัง การตัดเกณฑ์ที่ -55 dBm จึงช่วยป้องกันไม่ให้เพื่อนที่นั่งโต๊ะข้างๆ หรือนั่งหลังห้องแอบเช็กชื่อเข้ามาได้

- เผื่อความผันผวนของสัญญาณ (Fading Buffer) เป็นค่าที่ไม่ตึงเกินไปจนทำให้เครื่องที่วางบนโต๊ะเช็กชื่อไม่ผ่านจากปัญหารบกวนเล็กๆ น้อยๆ แต่ก็ไม่หย่อนเกินจนข้ามรัศมีโต๊ะปฏิบัติการ
```
...
```
ปัจจุบัน ESP32 ทำหน้าที่เป็น SoftAP (แจก Wi-Fi เอง) จึงไม่มีอินเทอร์เน็ต หากต้องการส่งข้อมูลขึ้น Cloud สามารถต่อยอดได้ 2 แนวทางหลัก

แนวทางที่ 1 เปลี่ยน ESP32 เป็น Station Mode หรือ Wi-Fi dual mode (AP + STA)

ฮาร์ดแวร์/เครือข่าย เพิ่มการเชื่อมต่อ Wi-Fi ของคณะ/มหาวิทยาลัย ให้ ESP32 ออกอินเทอร์เน็ตได้

ซอฟต์แวร์ฝั่ง ESP32

- เพิ่มไลบรารี esp_http_client หรือ HTTPS Client (พร้อม TLS/SSL Certificate)

- เขียนโค้ดส่งข้อมูลเมื่อมีคนเช็กชื่อผ่าน โดยยิง Request ไปยัง Google Apps Script Web App (สำหรับ Google Sheets) หรือยิง REST API ไปยัง Firebase Realtime Database

แนวทางที่ 2 ใช้มือถือของนักเรียนเป็นตัวกลางส่งขึ้น Cloud (Client-side Forwarding)

- แนวคิด ให้ ESP32 ส่งคำตอบเป็น JSON หรือ Redirect กลับไปหาเบราว์เซอร์ของนักเรียนเมื่อเช็กชื่อสำเร็จ

- ซอฟต์แวร์ฝั่งเว็บ (JavaScript/AJAX) บนหน้าเว็บที่ ESP32 ส่งไป ให้เขียน JavaScript ฝั่ง Client ให้เป็นผู้ส่งข้อมูล (Student ID, Timestamp) ยิงตรงเข้า Google Sheets / Firebase ผ่านเครือข่าย Cellular (4G/5G) ของตัวนักเรียนเอง วิธีนี้ ESP32 ไม่จำเป็นต้องต่ออินเทอร์เน็ตเลย
```
