...
```
มีระบบล็อคข้อมูลอัตโนมัติ ป้องกันปัญหาข้อมูลโดนเขียนทับครึ่งๆ กลางๆ ขณะสลับ Task (Thread-Safe)

มีระบบคิว (FIFO) เก็บข้อมูลได้หลายค่าตามลำดับ ไม่เหมือนตัวแปร Global ที่เก็บได้แค่ค่าปัจจุบันค่าเดียว

ไม่พัก CPU เปล่า (No Polling) Task ที่รอข้อมูลจะ "นอนรอ" ได้ทันที ไม่กินพลังงาน CPU เหมือนการสั่งวนลูปเช็กตัวแปร Global
```
2. ค่า **Stack High Water Mark** มีประโยชน์อย่างไรในการตรวจวินิจฉัยปัญหาบั๊กในระบบเรียลไทม์ (RTOS)?
```
เช็กพื้นที่ Stack ที่เหลือ ใช้บอกว่า Task นั้นๆ เหลือ Stack น้อยที่สุดเท่าไหร่ (ยิ่งใกล้ 0 ยิ่งอันตราย)

ป้องกันและปรับแต่ง ช่วยป้องกันบั๊ก Stack Overflow (ระบบค้าง/รีเซ็ต) และช่วยให้เราตั้งขนาด RAM ของ Task ได้พอดี ไม่มากหรือน้อยเกินไป
```
...
```
คิวจะเต็มแน่นอน (Queue Full)

วิธีรับมือขึ้นอยู่กับที่เราเขียนโค้ด

1. ทิ้งข้อมูลใหม่ ตั้งให้ข้ามการส่งถ้าคิวเต็ม (เหมาะกับงานที่ไม่ต้องการข้อมูลครบทุกจุด)

2. สั่งให้ Task ส่งนอนรอ ชะลอ Task เซ็นเซอร์ให้ทำงานช้าลงเท่า Task ส่ง Wi-Fi

3. เขียนทับข้อมูลเก่า ใช้ xQueueOverwrite เอาเฉพาะค่าล่าสุด

4. ยุบรวมข้อมูล ให้เซ็นเซอร์เก็บหาค่าเฉลี่ยก่อน แล้วค่อยส่งคิวทีเดียวทุก 500ms
```
